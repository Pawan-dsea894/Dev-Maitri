...
public class ApzDataConfiguration {
	...
	public LocalContainerEntityManagerFactoryBean apzEntityManagerFactory(EntityManagerFactoryBuilder builder,
			...
			@Qualifier("apzDataSource") DataSource apzDataSource, JpaProperties apzJpaProperties) {
		return builder.dataSource(apzDataSource).packages("com.iexceed.appzillonbanking.*.domain.apz")
				.persistenceUnit("apzdata")
				.properties(getJpaProperties(apzJpaProperties))
				.build();
	}
	...
	private Map<String, Object> getJpaProperties(JpaProperties jpaProperties) {
		...
		properties.put("hibernate.hbm2ddl.auto", apzJPADDLAuto);
		properties.put("hibernate.dialect", apzJPADialect);
		properties.put("hibernate.show_sql", apzShowSQL);
		properties.put("hibernate.format_sql", apzFormatSQL);
		return properties;
	}
