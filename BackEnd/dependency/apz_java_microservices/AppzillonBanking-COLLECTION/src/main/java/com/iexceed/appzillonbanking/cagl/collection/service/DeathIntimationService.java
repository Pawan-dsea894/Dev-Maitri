...
//								fetchDeathIntimationRequest.getAppId(),
//								APPLN_TYPE,
...
//			} else if ("DEO".equalsIgnoreCase(fetchDeathIntimationRequestFields.getKmUserRole())) {
//				applnMasterLst = applicationMasterRepository.findByBranchCodeAndApplicationType(
//						fetchDeathIntimationRequestFields.getBranchId(), APPLN_TYPE);
//			}
//			logger.debug("applnMasterLst is :: {} " , applnMasterLst);
...
//				logger.debug("applicationMasterMap is :: {} " , applicationMasterMap);
//				List<String> applicationIds = new ArrayList<>(applicationMasterMap.keySet());
//				logger.debug("applicationIds is :: {} " , applicationIds);
...
//							return responseMap;
//						}).toList();
//				logger.debug("deathIntimationResponseMap is :: {} ", deathIntimationResponseMap);
...
//	}
	...
	public Mono<Response> fetchDeathIntimation(FetchDeathIntimationRequestWrapper requestWrapper, Header header) {
		...
		try {
			...
			// fetching kendra info
			Mono<Object> fetchKendraInfoMono =
					interfaceAdapter.callExternalService(
							header, request, FETCH_KENDRA_INTF, true);
			return fetchKendraInfoMono.flatMap(val -> {
				logger.debug("fetchKendraInfo response :: {}", val);
				List<KendraDetailsDto> kendraList = getFetchKendraInfoResponse(val);
				...
				if ("KM".equalsIgnoreCase(requestFields.getKmUserRole())) {
					List<String> kendraIds = kendraList.stream()
							.map(k -> String.valueOf(k.getKendraId()))
							.toList();
					logger.debug("KM kendraIds :: {}", kendraIds);
					if (!kendraIds.isEmpty()) {
						applnMasterList = applicationMasterRepository
								.findByKendraIdInAndApplicationType(kendraIds, APPLN_TYPE);
					}
