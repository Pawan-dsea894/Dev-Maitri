...
import com.iexceed.appzillonbanking.cagl.entity.*;
...
public class FetchDetailsService {
	...
	@Autowired
	MahiLeadRepository leadRepository;
	...
	public AsmiUserResponse getUserDetailsData(GkKendraUserId userId) throws URISyntaxException {
		...
		logger.error("Fetched branchName & branchId based userId for URL is: {}", url);
		...
		try {
			...
			logger.error("Fetched branchName & branchId from ASMI table: {}", response);
	...
	public Map<String, Object> fetchCdhKendraDetailsForDeo(String userId,String branch, List<String> kList,
		...
		try {
			...
			logger.error("asmiUserResponse for DEO/BST: {}", asmiUserResponse);
			String designation;
			...
			if (asmiUserResponse == null || "APIIssue".equalsIgnoreCase(asmiUserResponse.getAddInfo1())
				...
				logger.error("userData for DEO/BST: {}", userData);
				...
				logger.error("CDH Hierarchy for DEO/BST : {}", designation);
				logger.error("CDH User Data for DEO/BST : {}", kendraList);					
			} else {	
				String userRole = asmiUserResponse.getAddInfo1();
				String branchId = asmiUserResponse.getAddInfo2();
				if(branch != null && !branch.trim().isEmpty()) {
					logger.error("BST User for branch : {}", branch);
					branchId=branch;
				}
				...
				logger.debug("ASMI asmiUserData : {}", asmiUserData);
				logger.error("ASMI asmiUserData : {}", asmiUserData);
				...
				logger.error("ASMI designation : {}", designation);
				logger.error("ASMI User Data: {}", kendraList);				
			}
		   ...
		   //  GkUserData userData = dao.fetchUserRole(userId);
			...
			for (GkKendraData gk : kendraList) {
				...
				/**
				 * @author Ankit.CAG Fetching Mahi-Leads count based on KendraId
				 */
				logger.debug("Fetching Mahi-LEAD count1");
				try {
					kendra.setMahiLeads(leadRepository.fetchMahiLeadCount(gk.getKendraId() + ""));
					logger.debug("Fetching Prnding Mahi-LEAD count1::{}");
					kendra.setPendingLeads(leadRepository.fetchPendingLeadCount(gk.getKendraId() + ""));
				} catch (Exception e) {
		...
		logger.error("Kendra Response object::{}", responseMap);
		return responseMap;
	}
	...
	Map<String, Object> responseMap = new HashMap<>();
		try {		
			List<KendraDataProjection> kendraList = dao.fetchKendraIdListDetailsNew(userId,CommonConstants.KENDRA_STATUS,kendraRequestField.getBranchId());	
			logger.debug("Printing kendraList for BM::{}", kendraList);
			if (kendraList != null && !kendraList.isEmpty()) {
	            responseMap.put("loans", kendraList);
	        } 			
		} catch (Exception exp) {
			logger.error(CommonConstants.EXCEP_OCCURED, exp);
		}
	...
	public Map<String, Object> fetchCdhKendraDetails(String userId, List<String> kList, List<String> excludedkList, String roleId,KendraRequestField kendraRequestField) { 
		...
		try {
			...
			logger.error("Printing asmiUserResponse for KM: {}", asmiUserResponse); 
			String designation;
			List<GkKendraData> kendraList = new ArrayList<>();
			if ("APIIssue".equalsIgnoreCase(asmiUserResponse.getAddInfo1())
				...
				logger.error("CDH data for KM: {}", designation);
				logger.error("CDH User Data for KM: {}", kendraList);
			} else {
				String userRole = asmiUserResponse.getAddInfo1();
				String branchId = asmiUserResponse.getAddInfo2();
				AsmiUserData asmiUserData = new AsmiUserData();
				...
				logger.error("ASMI designation for KM : {}", designation);
				logger.error("ASMI User Data for KM :  {}", kendraList);
			}
		 ...
		 //	List<GkKendraData> kendraList = dao.fetchKendraDetails(userId, userData, CommonConstants.KENDRA_STATUS, kList,excludedkList, roleId);
			...
			/**
			 * @author Ankit.CAG FETCH UNNATI GROUP NAME
			 */
			Map<Integer, List<GroupNameDto>> kendraGroupIds = new HashMap<>();
			try {
				List<GroupNameDto> groupNames = new ArrayList<>();
				logger.debug("Fetching unnatiGroupId Names::{}");
				if (customerListByKendra != null) {
					List<Integer> distinctGroupIds = customerListByKendra.values().stream().flatMap(List::stream)
							.map(CustData::getGroupId).filter(Objects::nonNull).distinct().collect(Collectors.toList());
					...
					kendraGroupIds = groupNames.stream().collect(Collectors.groupingBy(GroupNameDto::getkendraId));
				}
			} catch (Exception e) {
				logger.error("unnatiGroupId:" + CommonConstants.EXCEP_OCCURED, e);
			}
			// ----------END-----------
			for (GkKendraData gk : kendraList) {
				ResponseObject respobj = new ResponseObject();
				...
				kendra.setKendraId(gk.getKendraId());			
				 //@author Ankit.CAG				 
				List<GroupNameDto> groupList = kendraGroupIds.get(gk.getKendraId());
				if (groupList != null) {
					kendra.setUnnatiGroupDtls(groupList);
				} else {
					kendra.setUnnatiGroupDtls(Collections.emptyList());
				}
				// ----------END-----------							
				kendra.setKendraName(gk.getKendraName());
				kendra.setKmName(gk.getKmName());
								...
								.map(mahilead -> {
									...
									try {
										MahiLeadDto mahiLeadDto = MahiLeadDto.builder().branchId(mahilead.getBranchId())
												...
												.transactionId(mahilead.getTransactionId()).build();
